...
title8 <- tags$div(
  titlehtml, HTML("How did the percentage of homes with insufficient plumbing changes between 1970 and 2018 in NC's counties?")
)

leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  setView(-79, 36, zoom = 6) %>%
  addPolygons(
    data = plot8_data,
    stroke = TRUE,
...
popup11 <- paste0("<strong>", codes$County)
title11 <- tags$div(
...
leaflet() %>%
  ...
  addLegend(
    ...
    label = c("4,000 - 3,300%", "3,300 - 2,600%", "2,600 - 1,900%", "1,900 - 1,200", "1,200 - ", "Ds,", "s", "Sa"),
    position = "bottomright",
    opacity = 1
  ) %>%
